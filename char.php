...
function char_main(&$sqlr, &$sqlc)
{
    ...
    if ($sqlc->num_rows($result)) {
        ...
        $owner_acc_id = $sqlc->result(
            $result,
            0,
            'account'
        );
        $query = $sqlr->query(
            'SELECT username, SecurityLevel AS gmlevel FROM account LEFT JOIN account_access ON account.id=account_access.AccountID WHERE account.id = ' . $owner_acc_id . ' ORDER BY gmlevel DESC LIMIT 1'
        );
        $owner_name = $sqlr->result(
            $query,
            0,
            'username'
        );
        $owner_gmlvl = $sqlr->result(
            $query,
            0,
            'SecurityLevel'
        );
        ...
        if ($user_lvl >= $owner_gmlvl && (($side_v === $side_p) || !$side_v)) {
            $result = $sqlc->query(
                'SELECT characters.equipmentCache, BINARY characters.name AS name, characters.race, characters.class, characters.level, characters.zone, characters.map, characters.online, characters.totaltime, characters.gender, characters.account, character_stats.blockPct,
            ...
            if ($char['guildid'] && $char['guildid'] != 0) {
                ...
                $mrank = $char['rank'];
                $guild_rank = $sqlc->result(
                    $sqlc->query(
                        'SELECT rname FROM guild_rank WHERE guildid =' . $char['guildid'] . ' AND rid=' . $mrank . ''
                    ),
                    0,
                    'rname'
                );
            ...
            define(
                'CHAR_DATA_OFFSET_MELEE_HIT',
                0
            );
            define(
                'CHAR_DATA_OFFSET_SPELL_HEAL',
                1
            );
            define(
                'CHAR_DATA_OFFSET_SPELL_HIT',
                2
            );
            define(
                'CHAR_DATA_OFFSET_SPELL_HASTE_RATING',
                3
            );
            define(
                'CHAR_DATA_OFFSET_RESILIENCE',
                4
            );
            define(
                'CHAR_DATA_OFFSET_RANGE_HIT',
                5
            );
            $char_data = [
                0,
            ...
            */
            ...
            $sqlw = new SQL;
            $sqlw->connect(
                $world_db[$realmid]['addr'],
                $world_db[$realmid]['user'],
                $world_db[$realmid]['pass'],
                $world_db[$realmid]['name']
            );

            $equiped_items = [
                1  => [
                    '',
                    ($EQU_HEAD ? get_item_icon(
                        $EQU_HEAD,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_HEAD ? get_item_border(
                        $EQU_HEAD,
                        $sqlw
                    ) : 0),
                ],
                2  => [
                    '',
                    ($EQU_NECK ? get_item_icon(
                        $EQU_NECK,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_NECK ? get_item_border(
                        $EQU_NECK,
                        $sqlw
                    ) : 0),
                ],
                3  => [
                    '',
                    ($EQU_SHOULDER ? get_item_icon(
                        $EQU_SHOULDER,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_SHOULDER ? get_item_border(
                        $EQU_SHOULDER,
                        $sqlw
                    ) : 0),
                ],
                4  => [
                    '',
                    ($EQU_SHIRT ? get_item_icon(
                        $EQU_SHIRT,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_SHIRT ? get_item_border(
                        $EQU_SHIRT,
                        $sqlw
                    ) : 0),
                ],
                5  => [
                    '',
                    ($EQU_CHEST ? get_item_icon(
                        $EQU_CHEST,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_CHEST ? get_item_border(
                        $EQU_CHEST,
                        $sqlw
                    ) : 0),
                ],
                6  => [
                    '',
                    ($EQU_BELT ? get_item_icon(
                        $EQU_BELT,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_BELT ? get_item_border(
                        $EQU_BELT,
                        $sqlw
                    ) : 0),
                ],
                7  => [
                    '',
                    ($EQU_LEGS ? get_item_icon(
                        $EQU_LEGS,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_LEGS ? get_item_border(
                        $EQU_LEGS,
                        $sqlw
                    ) : 0),
                ],
                8  => [
                    '',
                    ($EQU_FEET ? get_item_icon(
                        $EQU_FEET,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_FEET ? get_item_border(
                        $EQU_FEET,
                        $sqlw
                    ) : 0),
                ],
                9  => [
                    '',
                    ($EQU_WRIST ? get_item_icon(
                        $EQU_WRIST,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_WRIST ? get_item_border(
                        $EQU_WRIST,
                        $sqlw
                    ) : 0),
                ],
                10 => [
                    '',
                    ($EQU_GLOVES ? get_item_icon(
                        $EQU_GLOVES,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_GLOVES ? get_item_border(
                        $EQU_GLOVES,
                        $sqlw
                    ) : 0),
                ],
                11 => [
                    '',
                    ($EQU_FINGER1 ? get_item_icon(
                        $EQU_FINGER1,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_FINGER1 ? get_item_border(
                        $EQU_FINGER1,
                        $sqlw
                    ) : 0),
                ],
                12 => [
                    '',
                    ($EQU_FINGER2 ? get_item_icon(
                        $EQU_FINGER2,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_FINGER2 ? get_item_border(
                        $EQU_FINGER2,
                        $sqlw
                    ) : 0),
                ],
                13 => [
                    '',
                    ($EQU_TRINKET1 ? get_item_icon(
                        $EQU_TRINKET1,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_TRINKET1 ? get_item_border(
                        $EQU_TRINKET1,
                        $sqlw
                    ) : 0),
                ],
                14 => [
                    '',
                    ($EQU_TRINKET2 ? get_item_icon(
                        $EQU_TRINKET2,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_TRINKET2 ? get_item_border(
                        $EQU_TRINKET2,
                        $sqlw
                    ) : 0),
                ],
                15 => [
                    '',
                    ($EQU_BACK ? get_item_icon(
                        $EQU_BACK,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_BACK ? get_item_border(
                        $EQU_BACK,
                        $sqlw
                    ) : 0),
                ],
                16 => [
                    '',
                    ($EQU_MAIN_HAND ? get_item_icon(
                        $EQU_MAIN_HAND,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_MAIN_HAND ? get_item_border(
                        $EQU_MAIN_HAND,
                        $sqlw
                    ) : 0),
                ],
                17 => [
                    '',
                    ($EQU_OFF_HAND ? get_item_icon(
                        $EQU_OFF_HAND,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_OFF_HAND ? get_item_border(
                        $EQU_OFF_HAND,
                        $sqlw
                    ) : 0),
                ],
                18 => [
                    '',
                    ($EQU_RANGED ? get_item_icon(
                        $EQU_RANGED,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_RANGED ? get_item_border(
                        $EQU_RANGED,
                        $sqlw
                    ) : 0),
                ],
                19 => [
                    '',
                    ($EQU_TABARD ? get_item_icon(
                        $EQU_TABARD,
                        $sqlm,
                        $sqlw
                    ) : 0),
                    ($EQU_TABARD ? get_item_border(
                        $EQU_TABARD,
                        $sqlw
                    ) : 0),
                ],
                ...
                while ($aura = $sqlc->fetch_assoc($a_results)) {
                    ...
                    $output .= '
                                            <a style="padding:2px;" href="' . $spell_datasite . $aura['spell'] . '" target="_blank">
                                                <img src="' . spell_get_icon(
                            $aura['spell'],
                            $sqlm
                        ) . '" alt="' . $aura['spell'] . '" width="24" height="24" />
                                            </a>';
                }
            }
            ...
            $output .= '
                    ...
                    $sqlm
                                        ...
                                        <br />' . $lang_char['guild'] . ': ' . $guild_name . ' | ' . $lang_char['rank'] . ': ' . htmlentities(
                    $guild_rank
                ) . '
            ...
            if (($user_lvl > $owner_gmlvl) || ($owner_name === $user_name)) {
                ...
                $tot_time -= ($tot_days * 86400);
                $total_hours = (int)($tot_time / 3600);
                $tot_time -= ($total_hours * 3600);
