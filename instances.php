...
use Application\Modules\InstanceModule;
...
//    $sqlw = new SQL;
//    $sqlw->connect(
//        $world_db[$realm_id]['addr'],
//        $world_db[$realm_id]['user'],
//        $world_db[$realm_id]['pass'],
//        $world_db[$realm_id]['name']
//    );
//
...
//    $all_record = $sqlw->result(
//        $sqlw->query('SELECT count(*) FROM instance_template'),
//        0
//    );
//
//    // main data that we need for this page, instances
//    $result = $sqlw->query(
//        'SELECT mapid, level_min, level_max, item_level, quest_done_A AS quest_a, quest_done_H AS quest_h, comment AS map FROM access_requirement
...
//        generate_pagination(
//            'instances.php?order_by=' . $order_by . '&amp;dir=' . (($dir) ? 0 : 1),
//            $all_record,
//            $itemperpage,
//            $start
//        );
...
$module = new InstanceModule(
    $realm_id,
    AbstractRepository::connectMySQL(
        AbstractRepository::convertArrayToCredentails($mmfpm_db)
    ),
    AbstractRepository::connectMySQL(
        AbstractRepository::convertArrayToCredentails(
            $world_db[$realm_id]
        )
    )
);

$output .= $module->renderTop($lang_instances);
$output .= $module->renderContent($lang_instances, $itemperpage);
